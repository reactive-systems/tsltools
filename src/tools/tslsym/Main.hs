...
main = do
  ...
  spec <- loadTSL input
  let table = toCSV $ symboltable spec
       ...
       in case head rs of
            "" ->
              let is' =
               ...
               in foldl (zipWith (\x y -> y ++ ';' : x)) is' a
            _ ->
              let is' =
                    if n > 1
                      then map (take (length (head is) - n + 1)) is
                      else is
               in rmSpaces (is' : a) $ map (\(';' : zs) -> zs) rs
