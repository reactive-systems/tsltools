...
testCoreQuery context minimalAssumptions tsl =
  let assmpt = assumptions tsl
   ...
   in do
        out <- incParallelFirst (threadPoolSize context) (fmap test subQueries)
        case out of
          Just True -> return Nothing
          _ -> do
            logNormal context $
              "Test Core with "
                ++ show (length (assumptions tsl))
                ++ " assumptions"
    ...
    genCore' minimalAssumptions (q : qr) = do
      ...
      case out of
        Nothing ->
          genCore'
            ( \set ->
                minimalAssumptions set
                  `union` ( if fromList (guarantees q) `isSubsetOf` set
