   ...
   in let directInputs =
       ...
       in unlines
            [ replicate 77 '-',
                ...
                ++ if null outputs
                  then "Output"
                    ...
                    "(Output signal "
                      ...
                      ++ ")",
              "",
              fName ++ " cell Functions{..} InitialState{..} Input{..} = do",
              "  rec",
              ...
              if null outputs
                then "  return"
                else "    let\n" ++ concatMap prSwitch outputs ++ "  return",
              "    Output",
              if null outputs
                then ""
                else
...
prCircuitImpl Circuit {..} =
  let (ls, ls') = unzip $ map latchDecl latches
      (os, os') =
        unzip $
   ...
   in unlines
        [ "controlCircuit",
          ...
          "  -> ControlIn signal -> m (ControlOut signal)",
          "",
          "controlCircuit cell ControlIn{..} = do",
          "  rec"
            ++ if null ls then "" else concatMap ("\n    " ++) ls ++ "\n",
          if null ds
            then ""
            else "    let" ++ concatMap ("\n      " ++) ds ++ "\n",
          "  return",
            ...
            then ""
              ...
              "      { controlOut0 = "
                ...
                ++ concatMap
                  ...
                  (zip [1 :: Int, 2 ..] $ tail os)
                ++ "\n     ( }",
          let hasLatches = not $ null latches
           ...
           in if hasLatches || hasGates || hasInverters
                then
                  "\n  where"
                    ++ ( if hasLatches
                           then "\n    _lat_ = cell False"
