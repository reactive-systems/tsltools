...
implement mName fName cfm@CFM {..} =
  ...
  where
    ...
    prSwitch o =
      indent 4 (outputName o)
        ++ "Out <-\n"
        ++ indent 6 (outputName o)
        ++ "Switch -<\n"
        ++ prMultiLineTuple
          8
          ( map
...
prCircuitImpl Circuit {..} =
  let (os, xs) = unzip $ map (\o -> polarized o 'o' $ outputWire o) outputs
   in unlines
        [ "controlCircuit",
          "  :: (Arrow signal, ArrowLoop signal)",
