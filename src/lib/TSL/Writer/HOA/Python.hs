...
printHOALines hoa@HOA {..} =
  let ?bounds = hoa
   in let -- TODO pretty sure this doesnt need to be sorted
          ...
          printLabel label stateUpdate =
            let splitFormulas = formulaToList label
                ...
                predUpdToCode (preds, upds) =
                  let conditional = if null preds then "True" else intercalate (" " ++ conjunctionOperator ++ indent 3) preds
                      body = indent 4 ++ intercalate (indent 4) (map updateToAssignment upds ++ [stateUpdate])
                   in "if (" ++ conditional ++ "):" ++ body
             in concatMap (\x -> indent 2 ++ predUpdToCode x) predUpds
...
updateToAssignment x =
  let constsSaved = T.unpack $ T.replace "()" "cccccc" $ T.pack x
   ...
   in T.unpack $ T.replace "cccccc" "()" $ T.concat [val, " = ", head fxnParts, params]
