...
statistics cfm@CFM {..} =
  let nI = length $ filter (not . loopedInput) inputs
   ...
   in (nI, length outputs, nP, nF, mC, mV)
  where
    inverters Circuit.Circuit {..} =
      let oW =
       ...
       in length $ mapMaybe isNegative $ toList $ unions [oW, lI, gA, gB]
...
symbolTable cfm@CFM {..} =
  let xs =
   ...
   in ST.symbolTable $ array (0, length xs - 1) $ sortIds xs
  where
    t2et = \case
    ...
    updOutputId =
      let -- get the non-looped outputs
          os =
            toList $
              difference
                (fromList outputs)
                (fromList (map snd loops))
       in -- create an update mapping to the compressed range
          (\x y -> assert (Map.member y x) (x Map.! y)) $
            Map.fromList $
              zip os [0, 1 .. length os - 1]
...
fromCircuit circuit = do
  ...
  let -- get all terms and their subterms
      ts =
        toList $
      ...
      ls =
        map
          ((`member` fromList (map fst loops)) . Input)
          [0, 1 .. length is - 1]
...
inferTypes cfm@CFM {..} = do
  ...
  where
    ...
    rearrange c@CFM {..} =
      let ps =
          ...
          upd = \case
            ...
            Poly i -> Poly $ f i
       in c
            { types = toList $ fromList $ map (upd . wireType' . wire) wires,
    ...
    compressed a = do
      ...
      let -- remove Boolean entries, double entries, and the constructor
          -- wrappers
          ts =
            rmDouble $
              map (\(Poly x) -> x) $
