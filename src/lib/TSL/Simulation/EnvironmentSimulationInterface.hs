...
--
-- The (command line) interface of the environment simulation when playing againts a 
-- strategy
--
-----------------------------------------------------------------------------
...
execSimulation :: EnvironmentSimulation -> IO ()
execSimulation sim = do
    ...
    (snd $
     foldl
       (\(n, xs) e ->
          ( n + 1
          , xs ++
            "  " ++ show n ++ " " ++ mappedPredicateEvaluationToString e ++ "\n"))
       (0 :: Int, [])
       posOpts)
  cPutStrLn Magenta $
    "Your turn now:\n" ++
  ...
  case act of
    Stop -> do
      cPutStrLn Red "You gave up! The system seems to be stronger than you."
      return ()
    Rewind -> do
      let sim' = rewind sim
      resetInterface
...
printTrace :: EnvironmentSimulation -> IO ()
printTrace sim = do
  let log = getLog sim
  _ <-
    sequence $
...
printImpossibleOptions ::
     (EnvironmentOption, [Formula String], [(String, SignalTerm String)])
  -> IO ()
