...
bindingError :: ExprPos -> String -> Either Error a
...
depError :: ExprPos -> String -> Either Error a
...
errUnknown :: String -> ExprPos -> StateT a (Either Error) b
errUnknown i pos =
...
errConflict :: String -> ExprPos -> ExprPos -> StateT a (Either Error) b
errConflict i x y =
  let msg =
...
errPattern :: ExprPos -> StateT a (Either Error) b
errPattern pos =
  let msg =
...
errConditional :: ExprPos -> StateT a (Either Error) b
errConditional pos =
...
errCircularDep :: [(String, ExprPos)] -> ExprPos -> Either Error b
...
errExpect :: ExprType -> ExprType -> ExprPos -> StateT a (Either Error) b
errExpect x y pos =
  ...
  where
    ...
    joinPoly im (rmS -> TPoly i) (rmS -> TPoly j) =
      case (readType im (TPoly i), readType im (TPoly j)) of
        ...
        (Left i', Right t) -> insert i t $ insert i' t $ insert j t im
        (Right t, Left j') -> insert i t $ insert j' t $ insert j t im
        _ -> im
...
-- to the range syntax.
errRange :: ExprType -> ExprPos -> StateT a (Either Error) b
errRange x pos =
  let msg =
...
-- format.
errFormat :: String -> Either Error a
errFormat = Left . ErrFormat . FormatError
-----------------------------------------------------------------------------
