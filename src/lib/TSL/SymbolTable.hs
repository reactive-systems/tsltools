...
toCSV ::
  SymbolTable -> String
toCSV SymbolTable {..} =
  let es = sortBy cmp $ assocs symtable
  ...
  where
    ...
    prExprPos = \case
      Nothing -> ""
      Just ExprPos {..} ->
        show (srcLine srcBegin)
          ++ ","
          ++ show (srcColumn srcBegin)
...
csvFormat ::
  [[String]] -> String
csvFormat xs =
  let -- get the maximal length of all entries for each column
